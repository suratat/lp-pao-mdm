...
| ไฟล์ | หน้าที่ |
|---|---|
| `docker-compose.staging.yml` | stack จริงตามภาคผนวก ค: postgres, migrate (one-shot), roles-bootstrap (one-shot), vault (raft), vault-init (one-shot, idempotent), api×2, worker, nginx, portal (T9 self-service), hr-console (T9 รอบ 2 — HR อนุมัติ claim request/ดู reverify list), dpo-console (T9 รอบ 3 — DPO/auditor ตรวจสอบ audit log) — ทั้งสาม console คุยกับ api ผ่าน service name `nginx` ใน network เดียวกัน ไม่ผ่าน localhost |
...
| `keycloak/realm-export.json` | realm `lp-pao` ตามภาคผนวก ก: client scopes (§2.2), audience mapper, clients (`check-broker`, `mdm-worker`, `mdm-portal`, `hr-console`, `dpo-console`, `eoffice`), realm roles |
