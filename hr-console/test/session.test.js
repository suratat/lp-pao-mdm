...
const sidFrom = (setCookies) => {
  const line = (setCookies || []).find((c) => c.startsWith('hr_console_sid=') && !c.startsWith('hr_console_sid=;'));
  return line ? line.split(';')[0].slice('hr_console_sid='.length) : null;
};
...
describe('1) ขนาด cookie: ไม่เกิน 4096 (และ ~200) ไบต์ แม้ token ใหญ่ผิดปกติ', () => {
  test('ผู้ใช้ที่มีทุก role + ทุก scope ของระบบ + role ปลอม 300 ตัว: ทุก Set-Cookie ตลอด login → หน้า → silent refresh → logout ไม่เกินเพดาน', async () => {
    ...
    const { agent, loginRes, callback, sid } = await login(harness.hrConsoleApp, 'huge-token-code');
    collect(loginRes);
    collect(callback);
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      const page = await agent.get('/hr/claim-requests');
    ...
    const sessionCookies = collected.filter((c) => c.startsWith('hr_console_sid='));
  ...
  test('ผู้ใช้ทั่วไป (token ปกติ) ก็ได้ cookie ขนาดเท่ากัน - ขนาดไม่ขึ้นกับ token', async () => {
    const { callback: small } = await login(harness.hrConsoleApp, 'good-code');
    const { callback: huge } = await login(harness.hrConsoleApp, 'huge-token-code');
    const size = (res) => bytes(res.headers['set-cookie'].find((c) => c.startsWith('hr_console_sid=')));
    expect(size(small)).toBe(size(huge));
  });

  test('ใช้ role/scope ครบตามที่ realm-export.json ประกาศจริง (ไม่ใช่รายการที่เดาเอง)', () => {
    expect(ALL_REALM_ROLES).toEqual(expect.arrayContaining(['hr_officer', 'hr_master_data_admin']));
    expect(ALL_CLIENT_SCOPES).toEqual(expect.arrayContaining(['personnel:provision', 'personnel:manage:reference', 'personnel:read:basic']));
  });
});
...
describe('2) silent refresh + single-flight (กัน invalid_grant จาก race)', () => {
  test('request พร้อมกัน 8 ตัวตอน token ใกล้หมดอายุ (refresh token ใช้ได้ครั้งเดียว) -> เรียก Keycloak refresh ครั้งเดียว ทุกตัวได้ 200 ไม่มี invalid_grant', async () => {
    const { agent } = await login(harness.hrConsoleApp, 'rotating-code');
    const stats = harness.mockKeycloak.stats;

    for (let wave = 1; wave <= 3; wave += 1) {
      const before = { ...stats };
      // eslint-disable-next-line no-await-in-loop
      const results = await Promise.all(Array.from({ length: 8 }, () => agent.get('/hr/claim-requests')));
  ...
  test('หลัง refresh: token ใน store เปลี่ยนเป็นตัวใหม่ (refresh token หมุนเวียน) และ session เดิมยังใช้ต่อได้', async () => {
    const { agent, sid } = await login(harness.hrConsoleApp, 'rotating-code');
    const before = harness.sessionStore.get(sid).refreshToken;
    expect((await agent.get('/hr/claim-requests')).status).toBe(200);
  ...
  test('refresh ถูก Keycloak ปฏิเสธ (invalid_grant) -> ลบ session, ล้าง cookie, redirect login', async () => {
    ...
    expect(res.headers['set-cookie'].some((c) => c.startsWith('hr_console_sid=;'))).toBe(true);
    expect(harness.sessionStore.get(sid)).toBeNull();
  });

  test('flag isMasterDataAdmin ยังอยู่หลัง silent refresh (หน้า master data ยังเข้าได้)', async () => {
    const { agent, sid } = await login(harness.hrConsoleApp, 'master-data-admin-short-code');
    for (let i = 0; i < 2; i += 1) {
      // eslint-disable-next-line no-await-in-loop
      const res = await agent.get('/hr/master-data/org-units');
      expect(res.status).toBe(200);
    }
    expect(harness.sessionStore.get(sid).isMasterDataAdmin).toBe(true);
  });
});
...
describe('3-4) logout, sid ปลอม/หมดอายุ/รูปแบบเก่า, session fixation', () => {
  test('logout ลบ session ฝั่งเซิร์ฟเวอร์: เอา sid เดิมไปใช้ซ้ำ (เช่น cookie ที่รั่ว) -> redirect login', async () => {
    const { agent, sid } = await login(harness.hrConsoleApp, 'good-code');
    expect(harness.sessionStore.get(sid)).not.toBeNull();
    await agent.get('/auth/logout');
    expect(harness.sessionStore.get(sid)).toBeNull();

    const replay = await request(harness.hrConsoleApp).get('/hr/claim-requests').set('Cookie', `hr_console_sid=${sid}`);
  ...
  test.each([
    ...
    const res = await request(harness.hrConsoleApp).get('/hr/claim-requests').set('Cookie', `hr_console_sid=${makeSid()}`);
    expect(res.status).toBe(302);
    expect(res.headers.location).toBe('/auth/login');
  });

  test('cookie รุ่นเก่า (hr_console_session JWE) ที่ค้างในเบราว์เซอร์ถูกเมินและถูกล้างตอน login/logout', async () => {
    ...
    const stale = `hr_console_session=${encodeURIComponent(legacyJwe)}`;
    const noSid = await request(harness.hrConsoleApp).get('/hr/claim-requests').set('Cookie', stale);
    ...
    const { agent, callback } = await login(harness.hrConsoleApp, 'good-code');
    expect(callback.headers['set-cookie'].some((c) => c.startsWith('hr_console_session=;') && c.includes('Max-Age=0'))).toBe(true);
    const out = await agent.get('/auth/logout');
    expect(out.headers['set-cookie'].some((c) => c.startsWith('hr_console_session=;'))).toBe(true);
    expect(out.headers['set-cookie'].some((c) => c.startsWith('hr_console_sid=;'))).toBe(true);
  });
  ...
  test('session fixation: login ได้ sid ใหม่เสมอ ไม่ยอมรับ sid ที่ผู้โจมตีตั้งมาก่อน และสองครั้งได้คนละค่า', async () => {
    ...
    const loginRes = await agent.set('Cookie', `hr_console_sid=${attackerSid}`).get('/auth/login');
    const state = new URL(loginRes.headers.location).searchParams.get('state');
    const cb = await request(harness.hrConsoleApp).get('/auth/callback').set('Cookie', `hr_console_oauth_state=${state}; hr_console_sid=${attackerSid}`).query({ code: 'good-code', state });
    ...
    const second = await login(harness.hrConsoleApp, 'good-code');
    expect(second.sid).not.toBe(newSid);
  });

  test('cookie session: HttpOnly, SameSite=Lax, Path=/, Max-Age=12 ชม.', async () => {
    const { callback } = await login(harness.hrConsoleApp, 'good-code');
    const line = callback.headers['set-cookie'].find((c) => c.startsWith('hr_console_sid='));
  ...
  test('callback ตั้งได้ทั้ง session cookie และล้าง state cookie พร้อมกัน (เดิม header ตัวหลังเขียนทับตัวแรก)', async () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['hr_console_oauth_state', 'hr_console_sid', 'hr_console_session']));
  });
});
