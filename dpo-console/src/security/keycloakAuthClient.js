...
// คุยกับ Keycloak (realm lp-pao) ตาม authorization code flow มาตรฐาน - client dpo-console เป็น
// confidential client (มี client secret) และ standardFlowEnabled=true (เหมือน hr-console ต่างจาก
// mdm-portal/mdm-worker ที่เป็น service account ล้วน) เพราะ DPO/auditor ต้องล็อกอินเป็นผู้ใช้จริงเพื่อให้
// ตรวจ realm role dpo/auditor ได้ (โครงสร้างไฟล์นี้เหมือน hr-console/src/security/keycloakAuthClient.js
// ทุกประการ - ไม่มีอะไรผูกกับ HR โดยเฉพาะ จึงคัดลอกมาแทนที่จะแชร์ผ่าน dependency ร่วม)
