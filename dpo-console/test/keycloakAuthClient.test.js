...
const CLIENT_ID = 'dpo-console-unit-test';
...
describe('keycloakAuthClient + idTokenVerifier (unit, ไม่พึ่ง Postgres/MDM API จริง)', () => {
  ...
  beforeAll(async () => {
    ...
    mockKeycloak = createMockKeycloakServer({
      auth,
      dpoConsoleClientId: CLIENT_ID,
      dpoConsoleClientSecret: CLIENT_SECRET,
      scenarios: {
        'dpo-code': { roles: ['dpo'], displayName: 'DPO หนึ่ง', username: 'dpo.one', scope: 'audit:read' },
        'auditor-code': { roles: ['auditor'], displayName: 'ผู้ตรวจสอบ', username: 'auditor.one', scope: 'audit:read' },
  ...
  test('buildLoginUrl ใส่ response_type/client_id/redirect_uri/scope/state ตาม contract', () => {
    ...
    expect(url.searchParams.get('scope')).toBe('openid audit:read');
    expect(url.searchParams.get('state')).toBe('state-123');
  });

  test('exchangeCode: code ที่ถูกต้อง -> ได้ access_token/refresh_token/id_token', async () => {
    const result = await authClient().exchangeCode('dpo-code');
  ...
  test('exchangeCode: client secret ผิด -> ok=false status 401', async () => {
    ...
    const result = await client.exchangeCode('dpo-code');
    expect(result.ok).toBe(false);
  ...
  test('refreshTokens: refresh token ที่ออกจาก exchangeCode ใช้ขอ token ใหม่ได้', async () => {
    const first = await authClient().exchangeCode('dpo-code');
  ...
  test('buildLogoutUrl ใส่ id_token_hint/post_logout_redirect_uri เมื่อมี logoutUrl ตั้งไว้', () => {
    const url = new URL(authClient().buildLogoutUrl('id-token-value', 'http://dpo-console.test/'));
    expect(url.pathname).toBe('/protocol/openid-connect/logout');
    expect(url.searchParams.get('id_token_hint')).toBe('id-token-value');
    expect(url.searchParams.get('post_logout_redirect_uri')).toBe('http://dpo-console.test/');
  });
  ...
  describe('idTokenVerifier', () => {
    ...
    test('id_token ของผู้มี role dpo -> isAllowed=true พร้อม displayName', async () => {
      const { tokens } = await authClient().exchangeCode('dpo-code');
      const identity = await verifier()(tokens.id_token);
      expect(identity.isAllowed).toBe(true);
      expect(identity.displayName).toBe('DPO หนึ่ง');
    });

    test('id_token ของผู้มี role auditor -> isAllowed=true เช่นกัน', async () => {
      const { tokens } = await authClient().exchangeCode('auditor-code');
      const identity = await verifier()(tokens.id_token);
      expect(identity.isAllowed).toBe(true);
    });

    test('id_token ของผู้ไม่มี role dpo/auditor -> isAllowed=false', async () => {
      const { tokens } = await authClient().exchangeCode('no-role-code');
      const identity = await verifier()(tokens.id_token);
      expect(identity.isAllowed).toBe(false);
    });

    test('id_token ที่ aud ไม่ตรงกับ client id ของ dpo-console -> ปฏิเสธ (throw)', async () => {
