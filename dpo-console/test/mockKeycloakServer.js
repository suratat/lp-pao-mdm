...
// จำลอง Keycloak เฉพาะส่วนที่ DPO Console คุยด้วยจริงคือ token endpoint (authorization_code +
// refresh_token grant) - เหมือน hr-console/test/mockKeycloakServer.js ทุกประการ (คัดลอกมาแทนที่จะแชร์
// ผ่าน dependency ร่วม เพราะแต่ละ console service เป็น deployable unit อิสระ)
//
// เซ็น id_token/access_token ด้วย private key เดียวกับที่ MDM API test instance เชื่อ (ctx.auth จาก
// api/test/testJwks.js) เพื่อให้ access_token ที่ DPO Console ส่งต่อไป MDM API จริงในเทสตรวจผ่านได้จริง -
...
function createMockKeycloakServer({ auth, dpoConsoleClientId, dpoConsoleClientSecret, scenarios }) {
  ...
  async function signIdToken(scenarioDef) {
    let jwt = new SignJWT({
      ...
      .setIssuedAt()
      .setSubject(scenarioDef.username || 'dpo-user')
      .setIssuer(auth.issuer)
      .setAudience(dpoConsoleClientId)
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      if (params.get('client_id') !== dpoConsoleClientId || params.get('client_secret') !== dpoConsoleClientSecret) {
